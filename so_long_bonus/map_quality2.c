...
/*   Updated: 2023/01/08 20:50:01 by mserrouk         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	map_alphabet(char **map)
{
	...
	while (map[i])
	{
		...
		while (map[i][j])
		{
			if (map[i][j] == 'E' || map[i][j] == 'C' || map[i][j] == 'R'
				|| map[i][j] == '1' || map[i][j] == '0' || map[i][j] == 'P')
				j++;
			else
			{
				ft_printf("Error\n");
				ft_printf("The map can be composed of only 6 characters:\n");
				ft_printf("'1','0','P','C','R' and 'E'\n");
				return (0);
			}
		}
		i++;
